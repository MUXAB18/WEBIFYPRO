...
const SERVICES = [
  { id: 'Web Development', icon: <Code2 size={28} />, color: 'var(--color-primary)', glow: 'rgba(11, 30, 57, 0.08)', label: 'Web Development', sub: 'React · Node.js · MERN' },
  { id: 'Digital Marketing', icon: <Megaphone size={28} />, color: 'var(--color-accent)', glow: 'rgba(255, 107, 53, 0.08)', label: 'Digital Marketing', sub: 'Meta Ads · Facebook · SEO' },
  { id: 'Mobile Application', icon: <Smartphone size={28} />, color: 'var(--color-primary)', glow: 'rgba(11, 30, 57, 0.08)', label: 'Mobile Application', sub: 'Coming Soon', disabled: true },
  { id: 'Social Media Management', icon: <Cpu size={28} />, color: 'var(--color-accent)', glow: 'rgba(255, 107, 53, 0.08)', label: 'Social Media', sub: 'Instagram · TikTok · Growth' },
];
    ...
    <>
      <div className="wizard-shell" style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '28px 32px 0' }}>
          ...
          <div style={{ display: 'flex', gap: '0', marginBottom: '32px' }}>
            {STEPS.map((name, i) => {
              ...
              const active = i === step;
              return (
                <div key={name} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', position: 'relative' }}>
                  {/* Connector line */}
                  {i > 0 && (
                    <div style={{
                  ...
                  <div style={{
                    ...
                    background: done ? 'var(--color-primary)' : active ? 'rgba(11, 30, 57, 0.05)' : '#fff',
                    ...
                    {done
                      ...
                      : <span style={{ fontSize: '0.75rem', fontWeight: '700', color: active ? 'var(--color-primary)' : 'var(--color-text)' }}>{i + 1}</span>
                    }
                  </div>
                  <span style={{
                    fontSize: '0.75rem', fontWeight: active ? '700' : '500',
                    color: active ? 'var(--color-primary)' : done ? 'var(--color-text)' : 'rgba(0,0,0,0.4)',
                    fontFamily: 'Outfit, sans-serif',
        ...
        <div 
          ...
          style={{ padding: '30px 40px', minHeight: '320px', position: 'relative', zIndex: 1, flex: 1 }}
        >
          {/* ─── STEP 0: Service Selection ─── */}
          {step === 0 && (
            <div>
              <h3 className="outfit" style={{ fontSize: '1.3rem', fontWeight: '800', color: 'var(--color-primary)', marginBottom: '6px' }}>
                What can we build for you?
              </h3>
              <p style={{ color: 'var(--color-text)', fontSize: '0.85rem', marginBottom: '22px' }}>
                ...
                {SERVICES.map(svc => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        padding: '18px 16px',
                        background: isActive ? `${svc.glow}` : '#ffffff',
                        border: `1.5px solid ${isActive ? svc.color : 'var(--color-border)'}`,
                        borderRadius: '14px',
                    ...
                    >
                      <div style={{ color: isActive ? svc.color : 'rgba(0,0,0,0.3)', transition: 'color 0.25s ease' }}>
                        {svc.icon}
                      </div>
                      <div>
                        <div style={{ fontSize: '0.88rem', fontWeight: '700', color: isActive ? 'var(--color-primary)' : 'var(--color-text)', fontFamily: 'Outfit, sans-serif' }}>
                          {svc.label}
                        </div>
          ...
          {step === 2 && (
            <div>
              <h3 className="wizard-title outfit" style={{ fontSize: '1.3rem', fontWeight: '800', color: 'var(--color-primary)', marginBottom: '6px' }}>
                Budget & Timeline
              </h3>
              <p style={{ color: 'var(--color-text)', fontSize: '0.85rem', marginBottom: '20px' }}>
                Help us tailor the right solution
              </p>

              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '0.8rem', fontWeight: '600', color: 'var(--color-primary)', fontFamily: 'Outfit, sans-serif', marginBottom: '10px' }}>
                  Estimated Budget
                </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          background: active ? 'rgba(11, 30, 57, 0.04)' : '#ffffff',
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                          <div style={{ fontSize: '0.78rem', fontWeight: '700', color: active ? 'var(--color-primary)' : 'var(--color-text)', fontFamily: 'Outfit, sans-serif' }}>
                            {b.label}
                          </div>
                          {active && (
                            <CheckCircle size={15} color="var(--color-primary)" style={{ animation: 'popIn 0.3s cubic-bezier(0.16,1,0.3,1) forwards' }} />
                          )}
                        </div>
                        <div style={{ fontSize: '0.68rem', color: active ? 'var(--color-primary)' : 'rgba(0,0,0,0.4)', fontFamily: 'Outfit, sans-serif' }}>
                          {b.price}
                        </div>
                      </button>
                    );
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px' }}>
                          {active && <CheckCircle size={13} color="var(--color-primary)" style={{ animation: 'popIn 0.3s cubic-bezier(0.16,1,0.3,1) forwards' }} />}
                          <div style={{ fontSize: '0.78rem', fontWeight: '700', color: active ? 'var(--color-primary)' : 'var(--color-text)', fontFamily: 'Outfit, sans-serif' }}>{t.label}</div>
                        </div>
                        <div style={{ fontSize: '0.64rem', color: active ? 'var(--color-primary)' : 'rgba(0,0,0,0.4)', marginTop: '2px' }}>{t.desc}</div>
                      </button>
                    );
                  })}
                </div>
            ...
            <div>
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={row.label} style={{
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--color-text)', fontFamily: 'monospace', minWidth: '70px', paddingTop: row.multiline ? '2px' : 0 }}>
                      {row.label}
                    </span>
                    <span style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: isSubmitting ? 'rgba(0,0,0,0.1)' : 'var(--color-accent)',
                  borderRadius: '12px', fontWeight: '700', fontSize: '1rem',
                  color: isSubmitting ? 'var(--color-text)' : 'white', border: 'none',
        ...
        <div className="wizard-footer-area" style={{
          ...
          <button
            ...
            style={{
              ...
              background: step === 0 ? 'transparent' : 'rgba(11, 30, 57, 0.08)',
              border: 'none',
              color: step === 0 ? 'transparent' : 'var(--color-primary)',
              cursor: step === 0 ? 'default' : 'pointer',
              fontWeight: '700', fontSize: '0.85rem',
              display: 'flex', alignItems: 'center', justifyContent: 'center', width: '120px',
              transition: 'all 0.3s ease',
              fontFamily: 'Outfit, sans-serif',
            ...
            <button
              ...
              style={{
                ...
                background: canProceed()
                  ...
                  : 'rgba(11, 30, 57, 0.08)',
                border: 'none',
                color: canProceed() ? '#fff' : 'rgba(11, 30, 57, 0.3)',
      ...
      <style>{`
        ...
        .wizard-input { background: #ffffff !important; border-color: var(--color-border) !important; color: var(--color-text) !important; }
        .wizard-input::placeholder { color: rgba(11, 30, 57, 0.3) !important; }
        .wizard-input:focus {
          border-color: var(--color-primary) !important;
          box-shadow: 0 0 0 1px var(--color-primary) !important;
        }
        .wizard-service-btn:hover { border-color: var(--color-border) !important; background: rgba(11, 30, 57, 0.02) !important; }
        .wizard-option-btn:hover { border-color: var(--color-border) !important; background: rgba(11, 30, 57, 0.02) !important; }
        .wizard-nav-btn:not(:disabled):hover { background: rgba(11, 30, 57, 0.05) !important; color: var(--color-primary) !important; }
        .wizard-next-btn:not(:disabled):hover { opacity: 0.9 !important; transform: translateY(-1px); }
        .launch-btn:not(:disabled):hover { opacity: 0.9 !important; transform: translateY(-2px); }
